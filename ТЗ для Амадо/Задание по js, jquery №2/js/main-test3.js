...
function superFunc(data) {
    data.forEach((el, index) =>  {
        if (data[el.id]) {
            ...
            list[index] = {'parent_id': el.id, 'name': el.name};
        }
    });
}
...
function categoryFunc(data) {
    for (let x in list) {
        if (boxes[list[x].id]) {
            ...
            boxes[list[x].id].appendChild(h4);
            boxes[list[x].id].appendChild(ul);
            data[x] = ul;
        } else {
            arr[x] = {'parent_id': list[x].id, 'name': list[x].name};
        }
    }
}
